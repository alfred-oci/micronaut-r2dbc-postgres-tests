...
import io.micronaut.data.runtime.operations.internal.*;
...
@Internal
@Replaces(DefaultR2dbcRepositoryOperations.class)
@Requires(condition = EnableR2dbcV1Compatibility.class)
final class DefaultR2dbcRepositoryOperations_WithCompatibility extends AbstractSqlRepositoryOperations<Connection, Row, Statement, RuntimeException> implements BlockingReactorRepositoryOperations, R2dbcRepositoryOperations, R2dbcOperations, ReactiveTransactionOperations<Connection> {
    private static final Logger LOG = LoggerFactory.getLogger(DefaultR2dbcRepositoryOperations_WithCompatibility.class);
     ...
     */
    @Internal
    protected DefaultR2dbcRepositoryOperations_WithCompatibility(
    ...
    private final class DefaultR2dbcReactiveRepositoryOperations implements ReactorReactiveRepositoryOperations {
        ...
        public <T, R> Mono<R> findOne(@NonNull PreparedQuery<T, R> preparedQuery) {
            ...
            })).as(DefaultR2dbcRepositoryOperations_WithCompatibility::toSingleResult);
        }
                ...
                return Flux.from(statement.execute())
                        .flatMap((result) -> Flux.from(result.getRowsUpdated()).flatMap(rowsUpdated -> {
                            ...
                            if (preparedQuery.isOptimisticLock()) {
                                checkOptimisticLocking(1, rowsUpdated.intValue());
                            }
        ...
        protected void executeUpdate(Statement stmt, DBOperation2<Integer, Integer, RuntimeException> fn) {
            data = data.flatMap(d -> Flux.from(stmt.execute()).flatMap(result -> Flux.from(result.getRowsUpdated()))
                    ...
                    .map(rowsUpdated -> {
                        ...
                        fn.process(1, rowsUpdated.intValue());
                        return d;
                    }));
        }
        ...
        protected void executeUpdateSetGeneratedId(Statement stmt) throws RuntimeException {
            data = data.flatMap(d -> {
                ...
                return Flux.from(stmt.execute()).flatMap(result ->
                        ...
                        .as(DefaultR2dbcRepositoryOperations_WithCompatibility::toSingleResult).map(id -> {
    ...
    private final class R2dbcEntitiesOperations<T> extends EntitiesOperations<T> {
        ...
        protected void executeUpdate(Statement stmt, DBOperation2<Integer, Integer, RuntimeException> fn) {
            Mono<Tuple2<List<Data>, Long>> entitiesWithRowsUpdated = entities.collectList()
                    .flatMap(e -> {
                        ...
                        return Flux.from(stmt.execute()).flatMap(result -> Flux.from(result.getRowsUpdated())).reduce(0L, Long::sum)
                                .map(rowsUpdated -> {
                                    fn.process(notVetoedEntities.size(), rowsUpdated.intValue());
        ...
        protected Mono<Long> getRowsUpdated() {
